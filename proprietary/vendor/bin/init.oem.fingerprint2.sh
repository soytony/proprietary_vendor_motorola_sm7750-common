...
function save_qseelog() {
	...
	SECURE_HARDWARE=$(getprop ${secure_property} 2> /dev/null)
    log "device secure status is: ${SECURE_HARDWARE}"
	# Check if the value is  "8" for no secure
	if [ "$SECURE_HARDWARE" = "8" ]; then
		timeout 1 cat /proc/tzdbg/qsee_log > /data/vendor/fps/qseelog
		log "QSEE log capture complete."
	fi
