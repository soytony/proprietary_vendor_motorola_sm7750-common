...
# roadstr case hall
count=0
while [ ! -e /sys/class/sensors/phone_case_hall_sensor ]; do
	sleep 1
	count=$(($count+1))
	if [ $count -gt $max ]; then
		break
...
if [ -e /sys/class/sensors/phone_case_hall_sensor ]; then
    chown root input /sys/class/sensors/phone_case_hall_sensor/enable
    chmod 0660 /sys/class/sensors/phone_case_hall_sensor/enable
    chown root input /sys/class/sensors/phone_case_hall_sensor/poll_delay
    chmod 0660 /sys/class/sensors/phone_case_hall_sensor/poll_delay
fi
